...
function buildPrompt(exercise: Exercise): string {
  ...
  return `Dynamic outdoor fitness photography shot from a low angle looking up. ${athlete} with a fit, athletic physique is ${exerciseDesc}. The athlete is completely barefoot, NO shoes, NO socks. CRITICAL ANATOMY: each foot has exactly 5 toes, each hand has exactly 5 fingers. The bare feet show natural toe splay with all five toes clearly visible and anatomically correct. The scene is shot outdoors ${ground} The background features lush green trees, foliage and bright sky with beautiful natural bokeh. The athlete is wearing only matte black athletic training shorts, no shirt, showing natural lean muscle definition. The lighting is bright natural daylight filtering through trees, with subtle rim light and backlight on the athlete's body. The overall mood is energetic, free and dynamic — a frozen split-second of powerful athletic movement in nature. Shot from a dramatic low angle perspective, sharp focus on the athlete with strong depth of field separation, natural warm color tones, 8k resolution, authentic outdoor sports photography.`;
}
