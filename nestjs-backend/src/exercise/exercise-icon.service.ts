...
function buildPrompt(exercise: Exercise): string {
  ...
  return `Dynamic outdoor fitness photography, full body shot with the ENTIRE person visible from head to feet — nothing cropped. ${athlete} with a slim, natural-looking physique (lean and toned, NOT bodybuilder, NOT overly muscular) is ${exerciseDesc}. The body posture must be anatomically correct and biomechanically accurate for this specific exercise — correct joint angles, proper spine alignment, and realistic weight distribution. The athlete is completely barefoot, NO shoes, NO socks. STRICT ANATOMY RULES: exactly 5 fingers on each hand, exactly 5 toes on each foot, no extra or missing digits. Hands and feet must be anatomically perfect. The scene is shot outdoors ${ground} The background features lush green trees, foliage and bright sky with beautiful natural bokeh. The athlete is wearing a fitted matte black athletic t-shirt and matching matte black training shorts — fully clothed, professional sportswear look. The lighting is bright natural daylight filtering through trees. The overall mood is natural, authentic and dynamic. IMPORTANT: frame the shot so the FULL BODY is visible including head and feet, with some space around the person. Natural warm color tones, 8k resolution, authentic outdoor sports photography.`;
}
