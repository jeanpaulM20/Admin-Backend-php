<script type="text/javascript">
	function processAction(type, id) {
		switch (type) {
			case 'update':
				window.location = '<?php echo $this->createUrl('account/update'); ?>/' + id;
				break;
			case 'delete':
				if (confirm('Do you really want to delete account with id ' + id)) {
					$.post('<?php echo $this->createUrl('account/delete'); ?>/' + id + '?ajax=1', {
...
$this->widget('zii.widgets.grid.CGridView', array(
	...
	'summaryText'			 => 'Displaying {start}-{end} of {count} result(s). Show items per page: ' . CHtml::dropDownList('pageSize', $pageSize, array(10	 => 10, 20	 => 20, 50	 => 50, 100	 => 100), array(
		'onchange'	 => "$.fn.yiiGridView.update('account-grid',{ data:{pageSize: $(this).val() }})",
	)),
	'columns'	 => array(
		array(
			'name'	 => 'client_search',
...
<?php
echo CHtml::htmlButton('New Account', array(
	'submit' => $this->createUrl('account/create', array('back' => 'admin'))
))
?>
...
<?php endif; ?>
						...
						$.fn.yiiGridView.doExport = function() {
							$.fn.yiiGridView.update('account-grid',{ 
								success: function() {
									$('#account-grid').removeClass('grid-view-loading');
									window.location = '<?php echo $this->createUrl('account/exportFile') ?>';
								},
								data: getData()
							});
						}
