<script type="text/javascript">
	function processAction(type, id) {
		switch (type) {
			case 'update':
				window.location = '<?php echo $this->createUrl('file/update'); ?>/' + id;
				break;
			case 'delete':
				if (confirm('Do you really want to delete file with id ' + id)) {
					$.post('<?php echo $this->createUrl('file/delete'); ?>/' + id + '?ajax=1', {
...
<?php echo CHtml::htmlButton('New File', array(
	'submit' => $this->createUrl('file/create', array('back' => 'admin'))
)) ?>
	...
	$(document).ready(function() {
		...
		$.fn.yiiGridView.doExport = function() {
			$.fn.yiiGridView.update('file-grid',{ 
				success: function() {
					$('#file-grid').removeClass('grid-view-loading');
					window.location = '<?php echo $this->createUrl('file/exportFile') ?>';
				},
				data: getData()
			});
		}
