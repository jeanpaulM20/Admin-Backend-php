<script type="text/javascript">
	function processAction(type, id) {
		switch (type) {
			case 'update':
				window.location = '<?php echo $this->createUrl('exercisesubgroup/update'); ?>/' + id;
				break;
			case 'delete':
				if (confirm('Do you really want to delete subgroup with id ' + id)) {
					$.post('<?php echo $this->createUrl('exercisesubgroup/delete'); ?>/' + id  + '?ajax=1', {
...
echo CHtml::htmlButton('New Exercise subgroup', array(
	'submit' => $this->createUrl('exercisesubgroup/create', array('back' => 'admin'))
))
?>
...
<?php endif; ?>
			...
			$.fn.yiiGridView.doExport = function() {
				$.fn.yiiGridView.update('exercisesubgroup-grid',{ 
					success: function() {
						$('#exercisesubgroup-grid').removeClass('grid-view-loading');
						window.location = '<?php echo $this->createUrl('exercisesubgroup/exportFile') ?>';
					},
					data: getData()
				});
			}
