...
?>
<h1>Avaliability calendar for <?php echo $trainer->getModelDisplay() ?></h1>
<div id="calendar"></div>
<?php
...
<?php 
echo CHtml::htmlButton('Serial input', array(
	'submit' => $this->createUrl('trainer/serialAvalibility') . '/' . $trainer->id
));
?>
	...
	$(document).ready(function() {
		...
		$('#calendar').fullCalendar({
			...
			events: function(start, end, callback) {
				$.ajax({
					url: '<?php echo $this->createUrl('trainer/listAvailability') . '?trainer_id=' . $trainer->id ?>',
					dataType: 'json',
					data: {
			...
			dayClick: function(date, allDay, jsEvent, view) {
				if (view.name == 'month') {
					...
					$('#updateForm').load('<?php echo $this->createUrl('form') ?>/?view=' + $('#calendar').fullCalendar('getView').name + '&trainer_id=<?php echo $trainer->id ?>', function() {
			...
			eventClick: function(calEvent, jsEvent, view) {
				$('#updateForm').load('<?php echo $this->createUrl('form') ?>/' + calEvent.id + '?view=' + $('#calendar').fullCalendar('getView').name + '&trainer_id=<?php echo $trainer->id ?>', function() {
					$('#avalability').dialog('open');
						jQuery('#TrainerAvailability_date')
							.datepicker({'showAnim':'fold','dateFormat':'dd-mm-yy'});
						jQuery('#TrainerAvailability_from')
