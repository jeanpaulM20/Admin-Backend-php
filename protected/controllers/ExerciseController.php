...
class ExerciseController extends Controller
{
	...
	public function actionCreate()
	{
		...
		// $this->performAjaxValidation(ch.esgroup.domainconverter.DomainModel@87e9bf);
	...
	public function actionUpdate($id)
	{
		...
		if (isset($_POST['Exercise'])) {
			$model->attributes = $_POST['Exercise'];
			if (!isset($_POST['Exercise']['exercisesets'])) {
				$model->exercisesets = array();
			}
	...
	public function gridGroupColumn($data, $row)
	{
		$valuesList = $this->getModel()->getGroupValues();
		return $valuesList[$data->group];
	}

	public function gridSubgroupColumn($data, $row)
	{
		$valuesList = $this->getModel()->getSubgroupValues();
		return $valuesList[$data->subgroup];
	}
	...
	public function actionExport()
	{
		...
		$headers = array(
			...
			array(
				'name'	 => 'group',
				'value'	 => array($this, 'gridGroupColumn'),
				'filter' => $this->getModel()->getGroupValues(),
			),
			array(
				'name'	 => 'subgroup',
				'value'	 => array($this, 'gridSubgroupColumn'),
				'filter' => $this->getModel()->getSubgroupValues(),
			),
		);
	...
	{
		Yii::app()->request->sendFile('Exercise.csv', Yii::app()->user->getState('export'));
