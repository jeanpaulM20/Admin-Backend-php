...
class ExercisesubgroupController extends Controller
{
	...
	{
		$model	 = new Exercisesubgroup;
		$oldData = array();

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation(ch.esgroup.domainconverter.DomainModel@87e9bf);

		if (isset($_POST['Exercisesubgroup'])) {
			$model->attributes = $_POST['Exercisesubgroup'];
			if ($model->save()) {
	...
	public function actionAdmin()
	{
		...
		$model				 = new Exercisesubgroup('search');
		$model->unsetAttributes();  // clear any default values
		if (isset($_GET['Exercisesubgroup']))
			$model->attributes	 = $_GET['Exercisesubgroup'];
	...
	{
		$model = Exercisesubgroup::model()->findByPk($id);
		if ($model === null)
	...
	{
		if (isset($_POST['ajax']) && $_POST['ajax'] === 'exercisesubgroup-form') {
	...
	{
		Yii::app()->request->sendFile('Exercisesubgroup.csv', Yii::app()->user->getState('export'));
