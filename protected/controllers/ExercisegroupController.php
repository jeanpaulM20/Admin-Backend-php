...
class ExercisegroupController extends Controller
{
	...
	{
		$model	 = new Exercisegroup;
		$oldData = array();

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation(ch.esgroup.domainconverter.DomainModel@87e9bf);

		if (isset($_POST['Exercisegroup'])) {
			$model->attributes = $_POST['Exercisegroup'];
			if ($model->save()) {
	...
	public function actionAdmin()
	{
		...
		$model				 = new Exercisegroup('search');
		$model->unsetAttributes();  // clear any default values
		if (isset($_GET['Exercisegroup']))
			$model->attributes	 = $_GET['Exercisegroup'];
	...
	{
		$model = Exercisegroup::model()->findByPk($id);
		if ($model === null)
	...
	{
		Yii::app()->request->sendFile('Exercisegroup.csv', Yii::app()->user->getState('export'));
