...
class TrainingController extends Controller
{
	...
	public function actionCreate()
	{
		...
		// $this->performAjaxValidation(ch.esgroup.domainconverter.DomainModel@fc40ae);
	...
	public function actionUpdate($id)
	{
		...
		if (isset($_POST['Training'])) {
			$model->attributes = $_POST['Training'];
			if (!isset($_POST['Training']['exercisesets'])) {
				$model->exercisesets = array();
			}
//			if (!isset($_POST['Training']['goals'])) {
//				$model->gaols = array();
//			}
			if ($model->save()) {
				$this->redirect(array('admin'));
	...
	{
		echo CJSON::encode(Training::getDropdownListByClientId($id));
	}
	/**
	...
	{
		$valuesList = $this->getModel()->getStatusValues();
		return $valuesList[$data->status];
	}
	...
	{
		Yii::app()->request->sendFile('Training.csv', Yii::app()->user->getState('export'));
