...
class Offer extends ActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('name, group, price', 'length', 'max' => 255),
			array('locations, id, name, group, type, teaser, file, price, startdate, startdate_range, enddate, enddate_range, archive, published, language_rel, language_search, trainer_id, trainer_search, parent_id', 'safe'),
			array('locations, id, name, group, type, teaser, file, price, startdate, startdate_range, enddate, enddate_range, archive, published, language_rel, language_search, trainer_id, trainer_search, parent_id', 'safe', 'on' => 'search'),
		);
	}
	...
	public function relations()
	{
		return array(
			...
			'locations' => array(self::MANY_MANY, 'Location', 'tbl_offer_location(offer_id, location_id)'),
			'language' => array(self::BELONGS_TO, 'Language', 'language_rel'),
			'parent' => array(self::BELONGS_TO, 'Offer', 'parent_id'),
			'children' => array(self::HAS_MANY, 'Offer', 'parent_id'),
		);
	}
	...
	public function search()
	{
		...
		$trainer_search		 = new CDbCriteria;
		$trainer_search->compare('parent.name', $this->parent_search, true, 'OR');
		$trainer_search->compare('parent.group', $this->parent_search, true, 'OR');
		$criteria->mergeWith($trainer_search);
		...
		return new CActiveDataProvider($this, array(
					...
					'sort'		 => array(
						'attributes' => array(
							...
							'parent_search' => array('asc'	 => 'parent.name,parent.group', 'desc'	 => 'parent.name DESC,parent.group DESC',),
							'*',
						),
					),
				));
