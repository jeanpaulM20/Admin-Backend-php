    ...
    private func stream(
        ...
        while !Task.isCancelled {
            do {
                ...
                // baseURL endet hier bereits auf /api/ (anders als in client-ios).
                guard let url = URL(string: "events/stream?channel=\(encodedChannel)&token=\(encodedToken)",
                                    relativeTo: APIConfig.baseURL) else { return }
